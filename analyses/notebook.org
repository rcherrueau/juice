...
def xp2orgtable(xps: List['XP']) -> List[List[str]]:
    def xp2orgtablerow(xp) -> List[str]:
        ...
        return [xp.cluster_size, latency, scn, xp.nosebleed, xp.success, fp]
    # Make org table
...
/concurrency/ of 10 specifies that the 100 iterations are executed by
10 users in a concurrent manner. The execution output may look like
this:
#+BEGIN_EXAMPLE
...
def make_xp(rally_path: str) -> XP:
    ...
    RE_XP_PARAMS = r'(?P<db>[a-z]+)-(?P<cluster_size>[0-9]+)-(?P<latency>[0-9]+)-(?P<nosebleed>[TF]).*'
...
#+NAME: lst:xps
#+BEGIN_SRC python :results silent
XPS = seq(MARIADB_XP_PATHS + GALERA_XP_PATHS + CRDB_XP_PATHS).truth_map(make_xp).cache()
#+END_SRC
...
def xp_csize_rtt_b_scn_order(xp: XP) -> str:
    ...
    return f'{xp.rdbms}-{xp.cluster_size:0>3}-{xp.latency:0>3}-{xp.nosebleed}-{xp.scenario}'
